...
# 
# flows.1819.csv <- c( "data/current/Actual Flow and Storage_2018_2019.csv",
#                      "data/current/Actual Flow and Storage_202001.csv" , 
#                      "data/current/Actual Flow and Storage_202002.csv")
...
#source('~/Dropbox/msandifo/documents/programming/r/2020/gasbb/r/gather_data.R')
...
# (actual.flows   %>%      subset(facilitytype=="PIPE" & state=="QLD") )$facilityname  %>% unique()
 ...
 actual.flows   %>%  
  ...
  ggplot(aes(gasdate, supply ))+
   geom_smooth(  col="lightblue2" , span=.035, method="loess", size=.5) +
   geom_line(size=.1, col="lightblue4") +
...
(p1.q <-ggplot(total.prod.month %>% subset(state=="QLD") %>%
               mutate(state = factor(state, levels=c(#"NSW",
                 "SA", "VIC", "QLD"))) , # %>%
             #subset (gasdate != g.date) , 
             aes(gasdate, supply , fill=state))+
    ...
    scale_y_continuous(  expand = c(0,0) )+
    scale_x_date(  expand = c(0,0) )+
    scale_x_date(limits=ymd(c("2009-01-01","2020-03-01")))+
...
ggsave(paste0('figs/auseast_coas_q.png'),  width=8.5, height=4.8)
...
(p1.v <-ggplot(total.prod.month %>% subset(state=="VIC") %>%
                 mutate(state = factor(state, levels=c(#"NSW",
                   "SA", "VIC", "QLD"))) , # %>%
               #subset (gasdate != g.date) , 
               aes(gasdate, supply , fill=state))+
    ...
    scale_y_continuous(  expand = c(0,0))+
    scale_x_date(  expand = c(0,0) )+
    scale_x_date(limits=ymd(c("2009-01-01","2020-03-05")))+
...
ggsave(paste0('figs/auseast_coas_v.png'),  width=8.5, height=4.8)
...
g.date.1 <- c(total.prod.month$gasdate[ind] , total.prod.month$gasdate[311], total.prod.month$gasdate[1071]) 
